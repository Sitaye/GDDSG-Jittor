...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, model_dir="./models"): # model_dir改成本地
    if cfg is None:
        cfg = getattr(model, 'default_cfg')
    if cfg is None or 'url' not in cfg or not cfg['url']:
        print("No pretrained weights URL/path found in config.")
        return

    filepath = os.path.join(model_dir, cfg['url'])

    if not os.path.exists(filepath):
        print(f"[ERROR] Weight file not found at: {filepath}")
        print("Please run the convert_weights.py script first!")
        return

    print(f"Loading .pth file from: {filepath}")
    
    state_dict = load_pytorch(filepath)
    ...
    print("Pretrained weights from .pth file loaded successfully!")
